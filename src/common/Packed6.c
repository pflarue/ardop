...
// three bytes using HART Packed-ASCII compression. The compressed
...
static bool compress_four_to_three(
	const char* uncompressed, uint8_t* compressed)
{
	...
	for (size_t ipos = 0; ipos < 4; ++ipos) {
		...
		//  * lower-numbered characters become spaces
		//  * lowercase → uppercase
...
// four ASCII characters using HART Packed-ASCII decompression. This
...
static void decompress_three_to_four(
	const uint8_t* compressed, char* uncompressed)
{
	const uint8_t SPACE = (uint8_t)' ';
	const uint32_t LOWER6 = (uint32_t)(0x3F);
	const uint8_t BYTE = (uint8_t)(0xFF);
...
bool packed6_from_str_slice(const char* str, size_t len, Packed6* packed) {
	...
	char work[9] = "";
	snprintf(work, sizeof(work), "%-8.*s", (int)len, str);
...
bool packed6_from_bytes(const void* bytes, size_t nbytes, Packed6* packed)
{
	if (nbytes != sizeof(packed->b))
	{
