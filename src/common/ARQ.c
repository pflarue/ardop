...
/**
 * @brief Timer for the packet LED.
 */
extern unsigned int PKTLEDTimer;

/**
 * @brief Buffer for data to be transmitted.
 */
extern UCHAR bytData[];

/**
 * @brief Quality of the last received frame.
 */
extern int intLastRcvdFrameQuality;

/**
 * @brief Measured leader length from the remote station.
 */
extern int intRmtLeaderMeasure;

/**
 * @brief Indicates whether an abort operation is in progress.
 */
extern bool blnAbort;

/**
 * @brief Counter for the number of repeated frames.
 */
extern int intRepeatCount;

/**
 * @brief Timeout for sending frames.
 */
extern unsigned int tmrSendTimeout;

/**
 * @brief Indicates whether a frame is pending for transmission.
 */
extern bool blnFramePending;

/**
 * @brief Timestamp of the last busy trip.
 */
extern int dttLastBusyTrip;

/**
 * @brief Timestamp of the prior last busy trip.
 */
extern int dttPriorLastBusyTrip;

/**
 * @brief Timestamp of the last busy clear.
 */
extern int dttLastBusyClear;

/**
 * @brief Timestamp of the last ID frame sent.
 */
extern unsigned int LastIDFrameTime;

/**
 * @brief Default delay for ARQ operations in milliseconds.
 */
extern int intARQDefaultDlyMs;

/**
 * @brief Average quality of received frames.
 */
int intAvgQuality = 0;

/**
 * @brief Pointer to the current data mode in the frame types for bandwidth.
 */
int intFrameTypePtr = 0;

/**
 * @brief Indicates whether the last frame sent was a data frame.
 */
bool blnLastFrameSentData = false;

/**
 * @brief Session bandwidth for the current ARQ connection.
 */
int intSessionBW = 0;

/**
 * @brief Indicates whether the ARQ connection is active.
 */
bool blnARQConnected = false;

/**
 * @brief Indicates whether a disconnect operation is in progress.
 */
extern bool blnARQDisconnect;

/**
 * @brief Holds the current ARQ protocol state.
 */
enum _ARDOPState ProtocolState;

/**
 * @brief Holds the current ARQ substate.
 */
enum _ARQSubStates ARQState;

/**
 * @brief Array of auxiliary callsigns for the ARQ session.
 */
StationId AuxCalls[AUXCALLS_ALEN];

/**
 * @brief Length of the auxiliary calls array.
 */
size_t AuxCallsLength = 0;

/**
 * @brief Holds the remote station's callsign in the current ARQ connection.
 */
StationId ARQStationRemote;

/**
 * @brief Holds the local station's callsign in the current ARQ connection.
 */
StationId ARQStationLocal;

/**
 * @brief Holds the final ID frame to send after the session ends.
 */
StationId ARQStationFinalId;

/**
 * @brief Indicates whether the ARQ session is pending.
 */
bool blnPending = false;

/**
 * @brief Timer for the final ID frame.
 */
unsigned int tmrFinalID = 0;

/**
 * @brief Timer for IRS pending timeout.
 */
unsigned int tmrIRSPendingTimeout = 0;

/**
 * @brief Timer for polling the outbound queue.
 */
unsigned int tmrPollOBQueue = 0;

/**
 * @brief Indicates whether DISC (disconnect) is repeating.
 */
bool blnDISCRepeating = false;

/**
 * @brief Holds the last received data frame type.
 */
UCHAR bytLastReceivedDataFrameType;

/**
 * @brief Holds the last ACKed data frame type.
 */
UCHAR bytLastACKedDataFrameType;

/**
 * @brief Holds the current frame type used for sending.
 */
UCHAR bytCurrentFrameType = 0;

/**
 * @brief Holds the byte array for data modes for a session bandwidth.
 */
UCHAR *bytFrameTypesForBW;

/**
 * @brief Length of the `bytFrameTypesForBW` array.
 */
int bytFrameTypesForBWLength = 0;

/**
 * @brief Holds the shift-up thresholds for ARQ sessions.
 */
UCHAR *bytShiftUpThresholds;

/**
 * @brief Length of the `bytShiftUpThresholds` array.
 */
int bytShiftUpThresholdsLength = 0;

/**
 * @brief Holds the session ID for the current ARQ connection.
 */
UCHAR bytSessionID = 0xFF;

/**
 * @brief Holds the pending session ID for the ARQ connection.
 */
UCHAR bytPendingSessionID;

/**
 * @brief Indicates whether the ARQ session is in listen mode.
 */
bool blnListen = true;

/**
 * @brief Indicates whether the ARQ session is in monitor mode.
 */
bool Monitor = true;

/**
 * @brief Indicates whether auto-break is enabled.
 */
bool AutoBreak = true;

/**
 * @brief Indicates whether a BREAK command is pending.
 */
bool blnBREAKCmd = false;

/**
 * @brief Indicates whether the channel is busy.
 */
bool BusyBlock = false;

/**
 * @brief Holds the number of bytes confirmed by ACK.
 */
int intBytesConfirmed = 0;

/**
 * @brief Holds the number of outbound bytes to confirm.
 */
int intOBBytesToConfirm = 0;

/**
 * @brief Holds the number of carriers in the current frame type.
 */
int GetNumCarriers(UCHAR bytFrameType);

/**
 * @brief Holds the average phase error creep per carrier.
 */
short dblAvgPECreepPerCarrier = 0;

/**
 * @brief Holds the total number of symbols processed.
 */
int intTotalSymbols = 0;

/**
 * @brief Holds the number of link turnovers in the ARQ session.
 */
int intLinkTurnovers = 0;
...
void SendData()
{
  ...
  case ISS:
    ...
    if (LastIDFrameTime != 0 && Now - LastIDFrameTime > 540000)
    {                       // 9 minutes
...
void ProcessRcvdARQFrame(UCHAR intFrameType, UCHAR *bytData, int DataLen, bool blnFrameDecodedOK)
{
  ...
  case DISC:
    ...
    if (IsCallToMe(&LastDecodedStationCaller, &LastDecodedStationTarget, &bytPendingSessionID)) // (Handles protocol rules 1.2, 1.3)
    {
      ...
      if (BusyBlock)
      {
        if ((blnLeaderTrippedBusy && dttLastBusyClear - dttPriorLastBusyTrip < 600) || (!blnLeaderTrippedBusy && dttLastBusyClear - dttLastBusyTrip < 600))
        {
  ...
  case IRS:
    ...
    if (ARQState == IRSData || ARQState == IRSfromISS) // Process Data or ConAck if ISS failed to receive ACK confirming bandwidth so ISS repeated ConAck
    {
      ...
      if (blnFrameDecodedOK && intFrameType == IDLEFRAME) // IF IDLE received from ISS
      {
        ...
        if ((AutoBreak && bytDataToSendLength > 0) || blnBREAKCmd || (LastIDFrameTime != 0 && Now - LastIDFrameTime > 540000) // 9 minutes, so BREAK
        )
        {
          // In an active ARQ Connection, only send an IDFrame when ProtocolState
          // is ISS or IDLE.  If ProtocolState is IRS and an IDLEFRAME is
