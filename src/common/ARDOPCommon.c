...
/**
 * @brief Called while waiting during TX to run background processes.
 *
 * This function performs tasks such as polling the TCP host, updating the Web GUI,
 * and polling received samples to prevent RX audio buffer overflow.
 *
 * @param mS The number of milliseconds to wait. If mS <= 0, the function returns quickly.
 */
void txSleep(int mS)
{
  ...
  while (Now < endNow && !blnClosing)
  {
    ...
    {
      PKTLEDTimer = 0;
      SetLED(PKTLED, 0); // Turn off packet received LED
    }
    TCPHostPoll();
    WebguiPoll();
...
/**
 * @brief Extends the RX WAV file recording time.
 *
 * This function updates the `rxwf_EndNow` variable to extend the recording time
 * of the RX WAV file.
 */
void extendRxwf()
{
...
/**
 * @brief Starts recording RX audio to a WAV file.
 *
 * Opens a new WAV file for recording RX audio if not already recording. If already
 * recording, extends the recording time. The WAV file name includes the host port,
 * UTC date, and UTC time.
 *
 * @note The WAV file contains only the RX audio, but continues to record while transmitting.
 * The audio recorded during transmission may contain noise or other artifacts. Because the
 * RX audio recrding is uninterrupted, the timing between various audio features should be
 * accurate. The absolute time at which any audio was received can be approximately calculated
 * based on the offset within the recording added to the time the recording started, which is
 * indicated in the filename.
 * The size of variable rxwf_pathname at least by 100 chars longer than the size of ArdopLogDir
 * in log.c.
 * The recording is stopped when the `StopRxWav()` function is called.
 */
void StartRxWav()
{
  ...
  if (rxwf != NULL)
  {
    if (!HWriteRxWav)
    {
      extendRxwf(); // Extend recording time when already recording
    }
    return;
  }
  ...
  wg_send_wavrx(0, true); // Update "RECORDING RX" indicator on Web GUI
  if (!HWriteRxWav)
  {
...
// audio is left in place but commented out so that it can easily
// be restored if desired.
/**
 * @brief Starts recording TX audio to a WAV file.
 *
 * Opens a new WAV file for recording filtered TX audio. The WAV file name includes
 * the host port, UTC date, and UTC time.
 */
void StartTxWav()
{
  ...
  // char txwfu_pathname[612]; // currently not used
  int pnflen;
  // int pnulen; // currently not used
  char timestr[16]; // 15 char time string plus terminating NULL
  get_utctimestr(timestr);
...
/**
 * @brief Creates a WAV file from a buffer of audio samples.
 *
 * This function is useful for diagnostic and debugging purposes. It creates a WAV file
 * from a short segment of audio data for later inspection and analysis.
 *
 * @param tag A string (max length 40) to include in the filename.
 * @param ptr Pointer to the buffer of audio samples.
 * @param num The number of 16-bit samples in the buffer.
...
int CreateWav(const char *tag, short *ptr, int num)
{
  ...
  if (strlen(tag) > 40)
  {
    ZF_LOGE("ERROR: tag=%s in CreateWav is too long. It must be 40 characters or less. WAV file not written.", tag);
    return -1;
  }
...
/**
 * @brief Sets the state of an LED and updates the GUI.
 *
 * @param LED The LED index to update.
 * @param State The new state of the LED (e.g., on or off).
 */
void SetLED(int LED, int State)
{
  Leds[LED] = State;
  SendtoGUI('D', Leds, 8); // Update LED state in GUI
}

/**
 * @brief Draws the current TX mode on the GUI.
 *
 * @param Mode The name of the TX mode to display.
 */
...
/**
 * @brief Draws the current TX frame on the GUI.
 *
 * @param Frame The name of the TX frame to display.
 */
...
/**
 * @brief Draws the current RX frame on the GUI.
 *
 * @param State The state of the RX frame (e.g., pending, good, bad).
 * @param Frame The name of the RX frame to display.
 */
...
/**
 * @brief Logs the current constellation data for debugging.
 *
 * This data can be copied from the debug log and used to reproduce the constellation plot.
 */
...
/**
 * @brief Sets a pixel in the constellation display.
 *
...
void mySetPixel(unsigned char x, unsigned char y, unsigned int Colour)
{
  ...
  if ((pixelPointer + 2 - Pixels) > (long int)sizeof(Pixels))
  {
    ...
    if (!overflowed)
      ZF_LOGW("WARNING: Memory overflow averted in mySetPixel. This warning is only logged once.");
    overflowed = true;
    return;
  }
...
/**
 * @brief Clears the constellation display.
 */
void clearDisplay()
{
  pixelPointer = Pixels; // Reset pixel pointer
}

/**
 * @brief Updates the constellation display.
 */
void updateDisplay()
{
...
/**
 * @brief Draws axes on the constellation display and logs the quality.
 *
 * @param Qual The quality value to display.
 * @param Mode The mode name to display.
 */
...
int decode_wav()
{
  ...
  while (true)
  {
    ...
    WavFileCount++;
    if (WavFileCount == sizeof(DecodeWav) / sizeof(DecodeWav[0]) || DecodeWav[WavFileCount][0] == 0x00)
      break;
  }
  return 0;
}
