// OS utility functions shared by the Unix-like platform backends.
//
// This file holds the parts of the platform OS-utility module that are
// identical for Linux (src/linux/os_util.c) and macOS (src/macos/os_util.c):
// timing, signal handling, serial (COM) port control, and TCP helpers.  The
// platform-specific pieces (CM108 HID PTT, audio-independent device
// enumeration, and Raspberry Pi GPIO) remain in the per-platform files.
