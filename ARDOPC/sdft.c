...
it only produces results for a single block of 240 (or 120) samples each time.
Thus, to get results for an adjacent overlapping block of 240 (or 120) tones,
such as might be useful to check or adjust symbol timing, would require repeated
execution of the full Goertzel algorithm.  In contrast, the SDFT algorithm
produces results for all overlapping blocks of samples at one sample intervals.
...
void init_sdft(int intCenterFrq, short * intSamples, int intDftLen)
{
	...
	// There is reset to FALSE at the end of demodulating the a frame type
