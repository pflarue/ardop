...
- Code layout: `src/common` cross-platform core (protocol, DSP, host/web interfaces); `src/linux` ALSA audio; `src/windows` WinMM audio; `src/macos` placeholder; web GUI assets in `webgui/` are embedded into C at build time.
...
- `make -j$(nproc)` builds ardopcf for the detected platform into `build/<platform>/ardopcf`.
...
- Sanity check: `./build/linux/ardopcf -h` or `build/windows/ardopcf.exe -h`.
- Cleaning: `make clean` removes platform build artifacts; `make cleanall` removes all of `build/`.

## Testing Instructions

- `make buildtest` compiles C unit tests; `make test` builds and runs them (cmocka required).
- CI (GitHub Actions) runs `make` then `make test` on Ubuntu (installs build-essential, libasound2-dev, libcmocka-dev) and on Windows via MSYS2 (gcc, mingw32-make, cmocka); artifacts zip only the built binary.
...
- Keep cross-platform logic in `src/common`; isolate platform-specific code in `src/linux` or `src/windows`; macOS code is minimal/experimental.
