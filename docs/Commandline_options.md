...
| **-K** | **&#8209;&#8209;kiss** | [&lt;address&gt;:]&lt;TCP port&gt; | Run a TCP [KISS](https://www.ax25.net/kiss.aspx) server on the given TCP port.  This allows external AX.25 software (such as APRS clients) to send and receive AX.25 frames using ardopcf as a simple FEC modem.  KISS data frames received from a connected client are decapsulated and the raw AX.25 frame they contain is transmitted using ARDOP FEC.  AX.25 frames decoded from received ARDOP FEC frames are KISS encapsulated and sent to all connected clients.  Each AX.25 frame is carried in a single ARDOP FEC frame, so the configured FEC mode (FECMODE) must be able to hold at least 256 bytes per frame (e.g. `16QAM.500.100`, `4FSK.2000.600`, or `16QAM.2000.100`); if it cannot, the KISS server will not start.  If the optional address is omitted, the server listens on loopback (127.0.0.1) only.  Use `0.0.0.0` to listen on all interfaces.  Enabling KISS forces FEC protocol mode.  Up to 16 simultaneous clients are supported. |
